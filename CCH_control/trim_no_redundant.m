...
Rotorcraft.DoubleRotorHelicopter.U         = 9;
...
x_trim_last = [0.01,0,0,0,0,0,10,10,1]; % 上一次的解
for j = 1:number_of_U
    disp(array_U(j))
    % 建立结构体(并行计算需要）
%     Rotorcraft = struct;
...
%     Rotorcraft.VerStab                  = VerStab;
    ...
    if exitflag > 0
        ...
        matrix_trim_states(j,:) = [array_U(j) nan*ones(1,28)];
    end
end
...
index = mod(array_U,step)==0;
array_U = array_U(index);
array_theta_0       = table_trim_states.theta_0(index);
array_theta_diff    = table_trim_states.theta_diff(index);
array_theta_1c      = table_trim_states.theta_1c(index);
array_theta_1s      = table_trim_states.theta_1s(index);
array_v_01          = table_trim_states.v_01(index);
array_v_02          = table_trim_states.v_02(index);
array_theta         = table_trim_states.theta(index);
array_phi           = table_trim_states.phi(index);
array_beta_01       = table_trim_states.beta_01(index);
array_beta_1c1      = table_trim_states.beta_1c1(index);
array_beta_1s1      = table_trim_states.beta_1s1(index);
array_beta_02       = table_trim_states.beta_02(index);
array_beta_1c2      = table_trim_states.beta_1c2(index);
array_beta_1s2      = table_trim_states.beta_1s2(index);
array_power_total_LowerRotor    = table_trim_states.power_total_LowerRotor(index);
array_power_total_UpperRotor    = table_trim_states.power_total_UpperRotor(index);
array_power_total_Prop          = table_trim_states.power_total_Prop(index);
array_power_total               = table_trim_states.power_total(index);

array_theta_preconfig = zeros(size(array_theta));
specified_last_theta = deg2rad(-3);
[number_of_theta,~] = size(array_theta);
for k = 1:number_of_theta
    maxmin = array_theta(11) - array_theta(end);
    move_up_percent = (specified_last_theta-array_theta(end))/maxmin;
    
    array_theta_preconfig(k) = array_theta(k) + move_up_percent*(array_theta(11)-array_theta(k));
end

array_theta_preconfig(1:11) = array_theta(1:11);
...
hold on
plot(array_U,rad2deg(array_theta_preconfig),'linewidth',1.5)
hold on
plot([70 70], get(gca, 'YLim'), '--r', 'LineWidth', 1)
hold off
xlabel('U'); ylabel('\theta(deg)'); title('U-\theta'); legend('\theta','\theta_{preconfig}');grid on;
subplot(2,1,2)
