...
for j = 1:number_sample
    ...
    time_estimated = time_average * (number_sample-j);
    if j < number_sample
        str=['生成样本15000_2中...',num2str(j),'/',num2str(number_sample),'（已用时 ',num2str(time_elapsed),'s，预计 ',num2str(time_estimated), 's后完成)'];
        %str=['生成样本中...',num2str(j),'/',num2str(number_sample)];    
        waitbar(j/number_sample,bar,str)                       % 更新进度条bar，配合bar使用
    else
        str='已完成！';   
        waitbar(j/number_sample,bar,str)                       % 更新进度条bar，配合bar使用
    end
...
writetable(table_trim_states,'trim_redundant_data_generation_15000_2.csv');