classdef RotorFixed < Helicopter
    ...
    methods
        ...
        function Power_induced    = get.Power_induced(obj)
            Power_induced = abs(obj.Z_h_blade_element)*obj.v_0 / obj.e_oswald;
        end
...
%             disp(DM0)
        ...
        function calculate_force(obj)
            ...
            C_Zhi   = -2*obj.lambda_i*sqrt(obj.mu^2+(obj.mu_z-obj.lambda_0)^2); %使用诱导速度算出的垂向力系数
            % 计算在桨毂系下的力
            X_h     = obj.rho*(obj.Omega*obj.R)^2*obj.A*C_Xh;
            Y_h     = obj.rho*(obj.Omega*obj.R)^2*obj.A*C_Yh;
            Z_h     = obj.rho*(obj.Omega*obj.R)^2*obj.A*C_Zh;
            Z_hi     = obj.rho*(obj.Omega*obj.R)^2*obj.A*C_Zhi; %使用诱导速度算出的垂向力
            
            Y_h = Y_h*obj.direction; %镜像转换
            
            obj.Z_h_blade_element       = Z_h;
            obj.Z_h_momentum_theory     = Z_hi;
