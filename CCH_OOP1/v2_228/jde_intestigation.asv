...
array_population_size       = 4:2:30;
[~,number_population_size]  = size(array_population_size);
matrix_power_best           = zeros(number_population_size,10);
matrix_time_elapsed         = zeros(number_population_size,10);
matrix_max_generation       = zeros(number_population_size,10);

for k = 1:number_population_size
    disp(array_population_size(k))
    % 每个population size 运行十次，分析结果是否一致
    parfor j = 1:10
