% 2.28 整机配平 带冗余变量 只有尾推 fmincon方法
clear all
clc
h = 100;
[~,~,~,rho] = atmosisa(h);
...
for j = 1:number_of_U
    ...
    array_redundant_var_best        = [0.30 1;0 0];
    ...
    if redundant_var_best(2) == 0
        problem.x0 = array_redundant_var_best(1,1);
    else
        problem.x0 = redundant_var_best(1);
    end
    
    [array_redundant_var_best(1,1) ,array_power_best(1)] = fmincon(problem);
    % 若无尾推
...
writetable(table_trim_states,'trim_result_redundent_prop_fmincon.csv');
