function F = AerodynamicsV3(States)
    % 王泽远 1.19
    % 无舵面，有尾推，平直平飞/悬停，诱导速度干扰，包含一阶谐波
...
%     v_01 = States(1) + 0*States(2);
%     v_02 = States(2) + 0*States(1);
    ...
    % 尾迹倾角,诱导速度畸变因子
    Chi1 = atan(mu_1/lambda_01);K1 = 15*pi/32*tan(Chi1/2);
    ...
    % 尾迹倾角,诱导速度畸变因子
    Chi2 = atan(mu_2/lambda_02);K2 = 15*pi/32*tan(Chi2/2);
    
    %诱导速度一阶余弦谐波
    v_1c1 = K1*States(1)+K2*States(2)*delta_1;
    v_1c2 = K2*States(2)+K1*States(1)*delta_2;
    
    % 下旋翼
    lambda_1c1 = v_1c1/(Omega*R); lambda_1s1 = v_1s1/(Omega*R); 
    ...
    % 上旋翼
    lambda_1c2 = v_1c2/(Omega*R); lambda_1s2 = v_1s2/(Omega*R); 
    ...
    if sqrt(u_HS^2+v_HS^2+w_HS^2) == 0
        beta_HS = 0;
    else
