function F = AerodynamicsV311(States)
    % 王泽远 1.20
    % 增加旋翼对平尾、垂尾的干扰
    % 平尾，垂尾使用naca0012,升力系数=5.7*迎角，阻力系数=0.008
    % 无舵面，有尾推，平直平飞/悬停，加入诱导速度干扰，但没有一阶谐波；
    
    % 王泽远 1.19
    % 无舵面，有尾推，平直平飞/悬停，加入诱导速度干扰，但没有一阶谐波
    % 注意此情况下，诱导速度与推力系数的关系有所改变 C_Zhi
    ...
    % 操纵变量 1c:纵向力 1s：横向力
    theta_01 = States(3)-States(4); theta_1c1=States(5); theta_1s1=States(6);
    ...
    %操纵变量 1c:纵向力 1s：横向力
    theta_02 = States(3)+States(4); theta_1c2=States(5); theta_1s2=States(6);
    ...
    v_01 = States(1) + delta_1*States(2);
    v_02 = States(2) + delta_2*States(1);
    ...
    if u_HS == 0
        ...
        beta_HS = atan(v_HS/1e-10);
    else
        alpha_HS = alpha_0F +  atan(w_HS/u_HS);  
        beta_HS = atan(v_HS/u_HS);
    end
    ...
    % 平尾气动力，力矩。 线性模型。 实际效果在大迎角（尤其是悬停时的-90度迎角）效果极差
    % C_LHS=3.5*alpha_HS;        % C_LHS=-3.5*alpha_HS;
    C_LHS=5.7*alpha_HS;          % naca0012
    %C_DHS=0;
    C_DHS=0.008;
    %不考虑升降舵
    ...
    if u_VS == 0
        alpha_VS = atan(v_VS/1e-8);
        beta_VS = asin(w_VS/1e-8);
    else
        alpha_VS = atan(v_VS/u_VS); 
        beta_VS = asin(w_VS/u_VS);
    end
    ...
    % 垂尾气动力，力矩。线性模型。 实际效果在大迎角（尤其是悬停时的-90度迎角）效果极差
    % C_LVS=3.5*alpha_VS;         % C_LVS=-3.5*beta_VS;
    C_LVS=5.7*alpha_VS;           % naca0012
    % C_DVS=0;
    C_DVS=0.008; 
    ...
    Rotor.N_R1 = N_R1;
    Rotor.v_01 = v_01;
    Rotor.v_i01 = States(1);
    Rotor.delta_1 = delta_1;
    ...
    Rotor.N_R2 = N_R2;
    Rotor.v_02 = v_02;
    Rotor.v_i02 = States(2);
    Rotor.delta_2 = delta_2;
    ...
    Tail.M_HS = M_HS;
    Tail.N_HS = N_HS;
    Tail.alpha_HS = alpha_HS;
    Tail.beta_HS = beta_HS;
