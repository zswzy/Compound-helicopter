...
lambda_0 = v_0/(Omega*R); lambda_1c = v_1c/(Omega*R); lambda_1s = v_1s/(Omega*R); 
...
hM0 = zeros(3,1);
% disp(((2*theta_1c-lambda_1s+p_bar_H)*mu_y+(2*theta_1s-lambda_1c+q_bar_H)*mu_x))
...
% 合力
X_R = rho*(Omega*R)^2*A*(C_Xh*cos(gamma_s) - C_Zh*sin(gamma_s));
Y_R = rho*(Omega*R)^2*A*C_Yh;
Z_R = rho*(Omega*R)^2*A*(C_Xh*sin(gamma_s) + C_Zh*cos(gamma_s));
...
L_R = L_h*cos(gamma_s)-N_h*sin(gamma_s)+h_R*Y_R;
M_R = M_h-h_R*X_R+x_cg*Z_R;
