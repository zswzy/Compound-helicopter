function F = AerodynamicsV33(States)
    % 王泽远 1.20
    % 尾翼使用naca0012。升力线斜率5.7，阻力系数0.008
...
%     C_DHS=0;
    ...
    if u_VS == 0
        alpha_VS = atan(v_VS/1e-10);
        beta_VS = atan(w_VS/1e-10);
    else
        alpha_VS = atan(v_VS/u_VS); 
        beta_VS = atan(w_VS/u_VS);
    end
