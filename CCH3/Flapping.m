function [beta_0,beta_1c,beta_1s] = Flapping(u,v,w,theta_0,theta_1c,theta_1s,v_0)
%FLAPPING 此处显示有关此函数的摘要
%   此处显示详细说明
global rho
...
hM0(4) = -2*(q_bar_H-p_Hp/2)+n_beta*((1-e^2)*((1+e^2+1/2*mu^2+mu_x^2)*theta_1s+mu_x*mu_y*theta_1c)+8/3*(1-e^3)*mu_x*theta_0+...
...
%         (2-8/3*e+2/3*e^4)*mu_y*theta_t+2*(1-e^2)*(mu_z-lambda_0)*mu_y-(1-e^4)*(lambda_1c-q_bar_H));
% 
% hM0(3) = -2*(q_bar_H-p_Hp/2)+n_beta*((1-e^2)*((1+e^2+1/2*mu^2+mu_x^2)*theta_1s+mu_x*mu_y*theta_1c)+8/3*(1-e^3)*mu_x*theta_0+...
%         (2-8/3*e+2/3*e^4)*mu_x*theta_t+2*(1-e^2)*(mu_z-lambda_0)*mu_x-(1-e^4)*(lambda_1s-p_bar_H));
% 
% DM0 = zeros(3,3);
% DM0(1,1) = lambda_beta^2;
...
beta_1s = -beta_1cW*sin(phi_W)+beta_1sW*cos(phi_W);
