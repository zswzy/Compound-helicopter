...
% 上旋翼
theta_02 = States(3); theta_1c2=-States(4); theta_1s2=States(5);

%% 运动状态，下旋翼
u_H1=50; v_H1=0; w_H1=0;
...
if mu_x1 == 0
    phi_W1 = atan(mu_y1/1e-10);
else
    phi_W1 = atan(mu_y1/mu_x1);
end

Chi_W1 = atan(mu_1/(lambda_01-mu_z1));   

if Chi_W1 < pi/2
    lambda_1cW1 = lambda_01*tan(Chi_W1/2);
else
    lambda_1cW1 = lambda_01*cot(Chi_W1/2);
end

    
lambda_1c1 =  lambda_1cW1*cos(phi_W1);
lambda_1s1 =  lambda_1cW1*sin(phi_W1);

v_1c1 = lambda_1c1*Omega*R;
v_1s1 = lambda_1s1*Omega*R;
...
X_R1 = X_h1*cos(gamma_s) - Z_h1*sin(gamma_s);
Y_R1 = Y_h1;
Z_R1 = X_h1*sin(gamma_s) + Z_h1*cos(gamma_s);
...
if mu_x2 == 0
    phi_W2 = atan(mu_y2/1e-10);
else
    phi_W2 = atan(mu_y2/mu_x2);
end

Chi_W2 = atan(mu_2/(lambda_02-mu_z2));   

if Chi_W2 < pi/2
    lambda_1cW2 = lambda_02*tan(Chi_W2/2);
else
    lambda_1cW2 = lambda_02*cot(Chi_W2/2);
end

lambda_1c2 =  lambda_1cW2*cos(phi_W2);
lambda_1s2 =  lambda_1cW2*sin(phi_W2);

v_1c2 = lambda_1c2*Omega*R;
v_1s2 = lambda_1s2*Omega*R;
...
X_R2 = X_h2*cos(gamma_s) - Z_h2*sin(gamma_s);
Y_R2 = Y_h2;
Z_R2 = X_h2*sin(gamma_s) + Z_h2*cos(gamma_s);
