function [individual_best,fval_best,output] = jde_par_con(problem,options)
%DE 
%   input: problem
...
m = problem.dimension;
f = problem.objective; % with constraint
lb = problem.lb;
ub = problem.ub;
theta_0_lb = problem.theta_0_lb;
theta_0_ub = problem.theta_0_ub;
...
parfor i = 1:size_population
    ...
    [matrix_fval(generation,i), x_trim] = f(matrix_population(i, :, generation));
    if x_trim(1)<theta_0_lb || x_trim(1)>theta_0_ub
        matrix_fval(generation,i) = inf;
    end
end
...
while generation <= max_generation
    ...
    if generation >= 6 ...
       ...
       % 并行需要
        ...
        parfor i = 1:size_population
            % selection, evaluate
            [fval_crossover, x_trim] = f(matrix_crossover(i, :,generation));
            if fval_crossover <= matrix_fval(generation,i) && x_trim(1)>theta_0_lb && x_trim(1)<theta_0_ub
