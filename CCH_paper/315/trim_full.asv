...
%nearest_initial_no_redundant = [0.01,0,0,0,0,0,10,10,1];
...
array_Prop_theta_0  = linspace(deg2rad(30),deg2rad(70),40);
[~,number_Prop_theta_0] = size(array_Prop_theta_0);
array_power_total         = zeros(size(array_Prop_theta_0));
array_power_prop         = zeros(size(array_Prop_theta_0));
array_power_rotor         = zeros(size(array_Prop_theta_0));
array_power_reduced_by_prop         = zeros(size(array_Prop_theta_0));

%nearest_initial_no_redundant = table_trim_states{table_trim_states.U == fix(Rotorcraft.DoubleRotorHelicopter.U),2:9};
nearest_initial_no_redundant = [0.01,0,0,0,0,0,10,10,10];

% initial power(no propeller)
options                 = optimset('Display','iter','TolFun',1e-15,'Maxiter',100,'Algorithm','levenberg-marquardt' ,'MaxFunEvals',20000);
cell_InitialStates      = {nearest_initial_no_redundant [0.01,0,0,0,0,0,10,10,10],[0.01,0,0,0,0,0,3,3,10], ...
                            [0.1,0,0,0,0,0,10,10,10],[0.1,0,0,0,0,0,3,3,10], ...
                            [0.2,0,0,0,0,0,10,10,10],[0.2,0,0,0,0,0,3,3,10], ...
                            [0.3,0,0,0,0,0,10,10,10],[0.3,0,0,0,0,0,3,3,10]};
[x_trim,~,~,~,Rotorcraft,Fnet,power_total] = trim_solve(Rotorcraft, ...
                                ...
                                deg2rad(0), ...         % theta_1c_diff
                                deg2rad(0));            % theta_1s_diff
power_total_no_prop = power_total;                               

x_trim_last = nearest_initial_no_redundant;
for j = 1:number_Prop_theta_0
    ...
    cell_InitialStates      = {x_trim_last [0.01,0,0,0,0,0,10,10,10],[0.01,0,0,0,0,0,3,3,10], ...
                            [0.1,0,0,0,0,0,10,10,10],[0.1,0,0,0,0,0,3,3,10], ...
                            [0.2,0,0,0,0,0,10,10,10],[0.2,0,0,0,0,0,3,3,10], ...
                            [0.3,0,0,0,0,0,10,10,10],[0.3,0,0,0,0,0,3,3,10]};
    [x_trim,~,~,~,Rotorcraft,Fnet,power_total] = trim_solve(Rotorcraft, ...
                                    ...
                                    deg2rad(0));            % theta_1s_diff
    x_trim_last = x_trim;
    array_power_total(j) = power_total;
    array_power_prop(j) = Rotorcraft.Prop.Power_total;
    array_power_rotor(j) = Rotorcraft.LowerRotor.Power_total+Rotorcraft.UpperRotor.Power_total;
    
end

array_power_reduced_by_prop = power_total_no_prop - array_power_total;

figure(1)
yyaxis left;
plot(rad2deg(array_Prop_theta_0),array_power_total/1000,'linewidth',1.5)
hold on 
plot(rad2deg(array_Prop_theta_0),array_power_rotor/1000,'linewidth',1.5)
hold off
ylabel('Power (kW)');
yyaxis right;
plot(rad2deg(array_Prop_theta_0),array_power_prop/1000,'linewidth',1.5)
ylabel('Propwller power (kW)');
legend('Total power','Rotor power','Propeller power','Location','best')
xlabel('Prop \theta_0 (deg)');title(['Power, U = ',num2str(Rotorcraft.DoubleRotorHelicopter.U)]);grid on;


figure(2)
plot(rad2deg(array_Prop_theta_0),array_power_reduced_by_prop/1000,'linewidth',1.5)
hold on
plot(get(gca, 'XLim'), [0,0], '--k', 'LineWidth', 1.5)
hold off
xlabel('Prop \theta_0 (deg)');ylabel('Power reduced (kW)');title(['Power reduced by propeller, U = ',num2str(Rotorcraft.DoubleRotorHelicopter.U)]);grid on;
...
% if propeller is NOT enabled
if Rotorcraft.DoubleRotorHelicopter.U < 50
    options.size_population     = 20;
    options.max_generation      = 100;
    options.scale_parameter     = 0.7;
    options.scale_parameter_lb  = 0.1;
