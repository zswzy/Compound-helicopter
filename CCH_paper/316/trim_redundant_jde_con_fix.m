% 3.8 Rotorcraft trim, full redundant (6 redundatn variables) for fixing,
% with constrain
% some point
clear all
clc
...
table_origin = readtable('trim_result_redundant_full_jde_con_fix_03_09_19_38.csv');
%% build object
run init_build.m
...
for j = 1:number_of_U
    ...
    if array_U(j) <= 50
        ...
        [array_redundant_var_best(2,3:6),array_power_best(2),~] = jde_par_con(problem,options);
    end
    ...
    if j < number_of_U
        str=['Ite ',num2str(j),'/',num2str(number_of_U),'(elapsed : ',num2str(time_elapsed),'s, remaining ',num2str(time_estimated), 's)'];
    else
        str='Finished!!!!!!!!';   
    end
...
filename = ['trim_result_redundant_full_jde_con_fix_',datestr(now,'mm_dd_HH_MM'),'.csv'];
