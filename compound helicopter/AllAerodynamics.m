function EQN = AllAerodynamics(States)
%Calculate the comprehensive forces and moments
...
X_F=0; Y_F=0; Z_F=0; L_F=0; M_F=0; N_F=0;
X_HS=0; Y_HS=0; Z_HS=0; L_HS=0; M_HS=0; N_HS=0;
X_VS=0; Y_VS=0; Z_VS=0; L_VS=0; M_VS=0; N_VS=0;
L_PR = 0;
% X_PR = 0;
...
% 需要12个补充方程
% States(1:20)需要求解，剩余为主函数预设值。
% States(1:20)中delta_e, delta_r, A_diff, B_diff, theta 为5个冗余变量
% 剩下共计15个~15个方程
...
hpT =  X_PR_I/4.445*v0_PR*(Omega_PR*R*3.28) / 550 + (rho*0.0019)*(A*3.28^2)*sigma_PR*(Omega_PR*R_PR*3.28)^2*c_d/8*(1+3*mu^2)/550;
...
F(1) = T_W1 - 2*rho*A*v_1*sqrt(u_H1H1^2+v_H1H1^2+(-w_H1H1+v_1+delta_l*v_2)^2);
F(2) = T_W2 - 2*rho*A*v_2*sqrt(u_H2H2^2+v_H2H2^2+(-w_H2H2+v_2+delta_u*v_1)^2);

end

function F = InducesVelocityP(vi)
%解算尾桨诱导速度
%   此处显示详细说明
global C_TPR lambda_0PR B_v_PR B_w_PR

v0_PR = vi;
F(1) = C_TPR - 4*v0_PR*sqrt((v0_PR-lambda_0PR)^2+B_v_PR^2+B_w_PR^2);
